class zombie extends GameObject {
  ...
  void act(){
    ...
    if(mapGrid[Math.max((int)(loc.x+dir.x+2000)/gridSize,0)][Math.max((int)(loc.z+dir.z+2000)/gridSize,0)] == 0 ){ //&& (Math.max((int)(loc.x+dir.x)/gridSize,0) != Math.max((int)loc.x/gridSize,0) || Math.max((int)(loc.z + dir.z)/gridSize,0) != Math.max((int)loc.z/gridSize,0))
    loc.add(dir);
    }
